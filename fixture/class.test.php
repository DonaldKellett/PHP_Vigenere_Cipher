...
class Test {
  ...
  public function assert_equals($actual, $expected, $msg = "Test Failed: Actual Value did not match Expected") {
    echo ($actual === $expected) ? "<p style='color:green;font-weight:bold;'>Test Passed: Value == $expected</p>" : "<p style='color:red;font-weight:bold;'>$msg => Expected: " . htmlspecialchars_decode("&quot;") . $expected . htmlspecialchars_decode("&quot;") . " but instead got: " . htmlspecialchars_decode("&quot;") . $actual . htmlspecialchars_decode("&quot;") . "</p>";
    if ($actual === $expected) {
      $this->passes++;
    } else {
      $this->fails++;
